...
from dagster.core.execution.plan.inputs import PendingStepInput, StepInput, UnresolvedStepInput
...
def _snapshot_from_step_input(step_input):
    check.inst_param(step_input, "step_input", (StepInput, UnresolvedStepInput, PendingStepInput))
    if isinstance(step_input, (UnresolvedStepInput, PendingStepInput)):
...
def _snapshot_from_execution_step(execution_step):
    check.inst_param(
        execution_step,
        "execution_step",
        (ExecutionStep, UnresolvedExecutionStep, PendingExecutionStep),
    )
