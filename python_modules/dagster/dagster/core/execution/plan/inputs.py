...
class FromPendingDynamicStepOutput(
    ...
    # is this right? should add a test
    def required_resource_keys(self, _pipeline_def: PipelineDefinition) -> Set[str]:
        return set()
...
class UnresolvedStepInput(
    NamedTuple(
        ...
        [
            ...
            (
                "source",
                Union[FromPendingDynamicStepOutput, FromUnresolvedStepOutput],
            ),
        ],
    )
):
...
class PendingStepInput(
    ...
    def __new__(
        ...
        return super(PendingStepInput, cls).__new__(
            cls,
            name=check.str_param(name, "name"),
            dagster_type_key=check.str_param(dagster_type_key, "dagster_type_key"),
            source=check.inst_param(source, "source", FromDynamicCollect),
        )
